...
Repeated audits cannot inflate or drown out the observation window: storage keeps only the latest snapshot for each audit frequency and business day, while trend criteria use the last measured state of each business day. Raw audit history remains available separately in `runtime.db`.
