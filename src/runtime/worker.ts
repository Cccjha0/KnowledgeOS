...
export async function executeTask(vaultRoot: string, repository: RuntimeRepository, task: RuntimeTask, workerId: string, resourcesChecked: JsonObject, handlers: Record<string, RuntimeHandler> = {}, moduleWorkflowHandler: RuntimeHandler = runModuleWorkflow): Promise<RuntimeTask> {
  ...
  try {
    ...
    if (classified.code === "OBSIDIAN_FILE_OPEN" || classified.code === "CONTEXT_BUDGET_REVIEW_REQUIRED" || classified.code === "CAPTURE_EXTRACTION_UNAVAILABLE") {
      const finished = repository.finishRun(run.run_id, {
        runStatus: "failed", taskStatus: "waiting-for-user", error: classified,
        metrics: { duration_ms: performance.now() - started }, completionReason: classified.code === "OBSIDIAN_FILE_OPEN" ? "obsidian-file-open" : classified.code === "CAPTURE_EXTRACTION_UNAVAILABLE" ? "capture-extraction-unavailable" : "context-budget-review-required",
      }).task;
      repository.recordMetricEvent({ idempotency_key: `${run.run_id}:waiting-for-user`, event_type: "task.waiting-for-user", module: task.module, instance_id: task.instance_id, workflow_id: String(task.trigger.workflow_id ?? task.workflow), workflow_version: typeof task.trigger.workflow_version === "string" ? task.trigger.workflow_version : null, prompt_id: null, prompt_version: null, run_id: run.run_id, occurred_at: new Date().toISOString(), dimensions: { reason: classified.code === "OBSIDIAN_FILE_OPEN" ? "obsidian-file-open" : classified.code === "CAPTURE_EXTRACTION_UNAVAILABLE" ? "capture-extraction-unavailable" : "context-budget-review-required" }, values: { duration_ms: performance.now() - started } });
      return finished;
    }
