...
export function classifyCodexFailure(message: string): { code: string; retryable: boolean } {
  ...
  if (/model.*(not found|unavailable|not supported|requires a newer version)|unsupported model/.test(normalized)) return { code: "CODEX_MODEL_UNAVAILABLE", retryable: false };
