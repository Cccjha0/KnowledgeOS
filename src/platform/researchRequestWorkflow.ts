...
import { exists, fromVaultPath, listFilesRecursive, toVaultPath, writeJsonAtomic } from "../core/files.js";
...
import { atPath, parseResearchRequestContract, startResearchRequestLifecycle, type ResearchRequestContract } from "../components/researchRequest.js";
...
export async function startResearchRequest(vaultRoot: string, moduleId: string, requestId: string, now = new Date().toISOString()): Promise<ResearchStartResult> {
  ...
  for (const instance of await discoverInstances(vaultRoot)) {
    if (instance.data.module_id !== moduleId || typeof instance.data.content_root !== "string") continue;
    const requestRoot = path.join(fromVaultPath(vaultRoot, instance.data.content_root), ...resolved.contract.request.directory.split("/"));
    if (!(await exists(requestRoot))) continue;
    candidates.push(...(await listFilesRecursive(requestRoot, ".md")).filter((file) => path.basename(file, ".md") === requestId));
  }
  ...
  const nextStatus = startResearchRequestLifecycle(resolved.contract, status);
  if (status === nextStatus) return { requestId, status: nextStatus, runId: null, planPath: null, snapshot: null, todayPath: toVaultPath(vaultRoot, await rebuildTodayDashboard(vaultRoot)) };
  ...
  const plan: OperationPlan = { plan_id: planId, task_id: taskId, source_module: resolved.id, instance_id: typeof instanceId === "string" ? instanceId : null, summary: `Mark Research Request ${requestId} in progress.`, review_items: [], operations: [{ operation_id: "OP-001", type: "update-frontmatter", target, risk: "green", confidence: 1, idempotency_key: `${resolved.id}:${requestId}:start`, payload: { patch: { [resolved.contract.request.status_field]: nextStatus, updated_at: now, next_action_at: now }, schema_id: resolved.contract.request.schema }, requires_review_id: null }] };
  ...
  return { requestId, status: nextStatus, runId, planPath: toVaultPath(vaultRoot, planPath), snapshot, todayPath: toVaultPath(vaultRoot, await rebuildTodayDashboard(vaultRoot)) };
}
