...
import { ensureDir, exists, listFilesRecursive, toVaultPath, writeJsonAtomic } from "../core/files.js";
...
export async function syncInstalledConfiguration(vaultRoot: string): Promise<ConfigurationSyncResult> {
  ...
  const previous = JSON.parse(await fs.readFile(path.join(vaultRoot, "90-System", "Modules", "installed.json"), "utf8").catch(() => "{\"modules\":[]}")) as { modules?: Array<{ id?: string; status?: string; version?: string; installed_path?: string; checksum?: string }> };
  const previousById = new Map((previous.modules ?? []).filter((item) => typeof item.id === "string").map((item) => [item.id!, item]));
  const previousStatus = new Map((previous.modules ?? []).filter((item) => typeof item.id === "string").map((item) => [item.id!, item.status]));
  const previousLock = JSON.parse(await fs.readFile(path.join(vaultRoot, "90-System", "Modules", "module-lock.json"), "utf8").catch(() => "{\"modules\":{}}")) as { modules?: Record<string, { installed_at?: string; version?: string; source?: string; installed_path?: string; checksum?: string; previous_version?: string | null; validation_report?: string }> };
  ...
  for (const manifestPath of (await listFilesRecursive(moduleRoot, "module.yaml")).filter((file) => path.basename(path.dirname(file)) !== "")) {
    ...
    officialIds.add(id);
    const destination = path.join(vaultRoot, "90-System", "Modules", "Official", id, version);
  ...
  for (const [id, entry] of previousById) {
    ...
    const lock = previousLock.modules?.[id];
    const installedPath = typeof entry.installed_path === "string" ? entry.installed_path : lock?.installed_path;
    if (!installedPath || installedPath.split("/").includes("..")) continue;
    const root = path.join(vaultRoot, ...installedPath.split("/"));
    if (!(await exists(path.join(root, "module.yaml")))) continue;
    modules.push({ id, version: String(entry.version ?? lock?.version ?? "0.0.0"), installed_path: installedPath, status: entry.status === "disabled" ? "disabled" : "enabled" });
    if (lock) moduleLock[id] = lock;
  }
