...
async function collectReviewDashboardItems(vaultRoot: string): Promise<DashboardItem[]> {
  ...
  for (const file of await listFilesRecursive(pendingRoot, ".md")) {
    ...
    items.push({
      ...
      target: toVaultPath(vaultRoot, file), due_at: null,
  ...
  for (const file of await listFilesRecursive(errorRoot, ".md")) {
    ...
    items.push({
      ...
      target: toVaultPath(vaultRoot, file), due_at: null, actions: ["open", "retry"],
    });
  }
  return items;
}
...
export async function buildTodayDashboard(vaultRoot: string, moduleItems: DashboardItem[] = []): Promise<string> {
  ...
  for (const item of items) validateSchema(vaultRoot, DASHBOARD_SCHEMA, item);
  items.sort((a, b) =>
    (PRIORITY_WEIGHT[a.priority] ?? 99) - (PRIORITY_WEIGHT[b.priority] ?? 99) || a.title.localeCompare(b.title),
  );
  ...
  const lines = ["# Today", "", `> 最后重建：${new Date().toISOString()}`, "", "## 今日概览", ""];
  if (items.length === 0) {
