...
test("F07 plugin contract includes shortcuts, bounded lists, accessible status, and offline fallback", async () => {
  ...
  assert.match(source, /codexModel: "gpt-5\.6-terra"/);
  assert.match(source, /codexReasoningEffort: "medium"/);
  assert.match(source, /invoke\("listCodexModels", \{\}\)/);
  assert.match(source, /supported_reasoning_efforts/);
  assert.match(source, /codex_model: this\.settings\.codexModel/);
  assert.match(source, /codex_reasoning_effort: this\.settings\.codexReasoningEffort/);
  assert.match(source, /back\.onclick = \(\) => this\.showCachedReviewList\(\)/);
  assert.match(source, /this\.pendingReviewActions\.set\(review\.review_id, params\.decision\)/);
