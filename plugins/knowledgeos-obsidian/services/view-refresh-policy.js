...
function affectedKnowledgeViews(filePath) {
  ...
  if (["90-System/Logs/", "90-System/Cache/", "90-System/Backups/"].some((prefix) => normalized.startsWith(prefix))) return [];

  if (normalized.includes("/Inbox/") || normalized.endsWith("/Inbox")) return [VIEW_KEYS.today, VIEW_KEYS.inbox];
  ...
  if (normalized.startsWith("90-System/State/")) {
    if (/\/(?:Captures|Sidecars)\//.test(normalized)) return [VIEW_KEYS.today, VIEW_KEYS.inbox];
...
module.exports = { VIEW_KEYS, affectedKnowledgeViews };
